...
const indentCode = function(codeStr) {
      ...
      styleguideArray.forEach((obj) => {
        ...
        renderer.code = function(snippet, language) {
          ...
          let fileName = `${frameName}-example-${exampleCount}.html`,
            ...
            highlighted = highlight.highlightAuto(indentCode(snippet)).value,
            code = template({
              snippet: snippet,
          ...
          fs.writeFileSync(`${options.output}/${fileName}`, code);
          exampleCount++;
          return `<section class="code-block cf"><iframe class="code-sample" id="${frameId}" src="${fileName}"></iframe>\n<code language='${language}' class="hljs" >${highlighted}</code>\n</section>`;
        };
